...
func (c *Client) AssignProfile(ctx context.Context, name, uuid string, serials ...string) (*AssignProfileResponse, error) {
	req := &ProfileServiceRequest{ProfileUUID: &uuid, Devices: serials}
	resp := new(AssignProfileResponse)
...
func (c *Client) RemoveProfile(ctx context.Context, name string, serials ...string) (*ClearProfileResponse, error) {
	req := &ClearProfileRequest{Devices: serials}
